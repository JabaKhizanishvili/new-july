import React from 'react';
// partners
// import partner1 from "/assets/images/partners/1.png";
// import partner2 from "/assets/images/partners/2.png";
// import partner3 from "/assets/images/partners/3.png";
// import partner4 from "/assets/images/partners/4.png";
// import partner5 from "/assets/images/partners/5.png";
// import partner6 from "/assets/images/partners/6.png";

// svg
import Svg1 from "/assets/svg/1.svg";
import Svg2 from "/assets/svg/2.svg";
import Svg3 from "/assets/svg/3.svg";

// resources
// import '/assets/images/resources/1.png' from "/assets/images/resources/1.png";
// import rsrc2 from "/assets/images/resources/2.png";
// import rsrc3 from "/assets/images/resources/3.png";

// import User from "/assets/images/other/2.png";

export const partners = [
    '/assets/images/partners/1.png',
    '/assets/images/partners/2.png',
    '/assets/images/partners/3.png',
    '/assets/images/partners/4.png',
    '/assets/images/partners/5.png',
    '/assets/images/partners/6.png',
];

export const levels = [
    {
        icon: <img style={{ width: "73px", }} className="group-hover:fill-white transition ease-in-out" src={Svg1} />,
        title: "Novice",
        paragraph: "You want to start trading but don't know hot to do it",
    },
    {
        icon: <img style={{ width: "73px", }} className="group-hover:fill-white transition ease-in-out" src={Svg2} />,
        title: "Beginner",
        paragraph:
            "The volume of redundant information has overwhelmed you when you tried in the past",
    },
    {
        icon: <img style={{ width: "73px", }} className="group-hover:fill-white transition ease-in-out" src={Svg3} />,
        title: "With some skills",
        paragraph:
            "Although you've been trading for some time, you can't seem to get your profits to exceed your looses",
    },
];

export const resources = [
    {
        title: "Video cources",
        icon: '/assets/images/resources/1.png',
        checks: ["Trading Essentials", "Technical Trading", "Strategic Trading"],
    },
    {
        title: "Market research",
        icon: '/assets/images/resources/2.png',
        checks: ["Daily Videos", "Market News", "Technical Analysis Articles"],
    },
    {
        title: "Tools",
        icon: '/assets/images/resources/3.png',
        checks: ["Economic Calendar", "Cryptocurrency Calendar"],
    },
];

export const currencyTabData = [
    {
        currency: "EUR",
        boxes: [
            {
                price: "€189",
                discount: "€250",
                head: "Beginner",
                checks: [
                    {
                        title: "Trading essentials course",
                        subtitle: [
                            "Technical trading couse",
                            "Strategic trading course",
                            "MT4 & MT5 tutorials",
                        ],
                    },
                    {
                        title: "eBooks",
                        subtitle: [
                            "3 high-probability trading setups",
                            "knowledge checks",
                            "Assignments",
                            "Economic calendar",
                            "Cryptocurrency calendar",
                            "Daily market news",
                        ],
                    },
                    {
                        title: "31 lessons",
                    },
                ],
            },
            {
                price: "€189",
                discount: "€250",
                head: "Intermediate",
                checks: [
                    {
                        title: "Trading essentials course",
                        subtitle: [
                            "Technical trading couse",
                            "Strategic trading course",
                            "MT4 & MT5 tutorials",
                        ],
                    },
                    {
                        title: "eBooks",
                        subtitle: [
                            "3 high-probability trading setups",
                            "knowledge checks",
                            "Assignments",
                            "Economic calendar",
                            "Cryptocurrency calendar",
                            "Daily market news",
                            "Assignments",
                            "Economic calendar",
                            "Cryptocurrency calendar",
                            "Daily market news",
                        ],
                    },
                    {
                        title: "31 lessons",
                    },
                ],
            },
            {
                price: "€189",
                discount: "€250",
                head: "Trader",
                checks: [
                    {
                        title: "Trading essentials course",
                        subtitle: [
                            "Technical trading couse",
                            "Strategic trading course",
                            "MT4 & MT5 tutorials",
                        ],
                    },
                    {
                        title: "eBooks",
                        subtitle: [
                            "3 high-probability trading setups",
                            "knowledge checks",
                            "Assignments",
                            "Economic calendar",
                            "Cryptocurrency calendar",
                            "Daily market news",
                        ],
                    },
                    {
                        title: "31 lessons",
                    },
                ],
            },
            {
                price: "€3799",
                discount: "€4799",
                head: "Elite",
                checks: [
                    {
                        title: "Trading essentials course",
                        subtitle: [],
                    },
                    {
                        title: "eBooks",
                        subtitle: [],
                    },
                    {
                        title: "eBooks",
                        subtitle: [],
                    },
                    {
                        title: "31 lessons",
                    },
                    {
                        title: "31 lessons",
                    },
                    {
                        title: "31 lessons",
                    },
                ],
            },
        ],
    },
    {
        currency: "USD",
        boxes: [
            {
                price: "$189",
                discount: "$250",
                head: "Beginner",
                checks: [
                    {
                        title: "Trading essentials course",
                        subtitle: [
                            "Technical trading couse",
                            "Strategic trading course",
                            "MT4 & MT5 tutorials",
                        ],
                    },
                    {
                        title: "eBooks",
                        subtitle: [
                            "3 high-probability trading setups",
                            "knowledge checks",
                            "Assignments",
                            "Economic calendar",
                            "Cryptocurrency calendar",
                            "Daily market news",
                        ],
                    },
                    {
                        title: "31 lessons",
                    },
                ],
            },
            {
                price: "$189",
                discount: "$250",
                head: "Beginner",
                checks: [
                    {
                        title: "Trading essentials course",
                        subtitle: [
                            "Technical trading couse",
                            "Strategic trading course",
                            "MT4 & MT5 tutorials",
                        ],
                    },
                    {
                        title: "eBooks",
                        subtitle: [
                            "3 high-probability trading setups",
                            "knowledge checks",
                            "Assignments",
                            "Economic calendar",
                            "Cryptocurrency calendar",
                            "Daily market news",
                        ],
                    },
                    {
                        title: "31 lessons",
                    },
                ],
            },
            {
                price: "$189",
                discount: "$250",
                head: "Beginner",
                checks: [
                    {
                        title: "Trading essentials course",
                        subtitle: [
                            "Technical trading couse",
                            "Strategic trading course",
                            "MT4 & MT5 tutorials",
                        ],
                    },
                    {
                        title: "eBooks",
                        subtitle: [
                            "3 high-probability trading setups",
                            "knowledge checks",
                            "Assignments",
                            "Economic calendar",
                            "Cryptocurrency calendar",
                            "Daily market news",
                        ],
                    },
                    {
                        title: "31 lessons",
                    },
                ],
            },
            {
                price: "$189",
                discount: "$250",
                head: "Beginner",
                checks: [
                    {
                        title: "Trading essentials course",
                        subtitle: [
                            "Technical trading couse",
                            "Strategic trading course",
                            "MT4 & MT5 tutorials",
                        ],
                    },
                    {
                        title: "eBooks",
                        subtitle: [
                            "3 high-probability trading setups",
                            "knowledge checks",
                            "Assignments",
                            "Economic calendar",
                            "Cryptocurrency calendar",
                            "Daily market news",
                        ],
                    },
                    {
                        title: "31 lessons",
                    },
                ],
            },
        ],
    },
    {
        currency: "GBP",
        boxes: [
            {
                price: "£189",
                discount: "£250",
                head: "Beginner",
                checks: [
                    {
                        title: "Trading essentials course",
                        subtitle: [
                            "Technical trading couse",
                            "Strategic trading course",
                            "MT4 & MT5 tutorials",
                        ],
                    },
                    {
                        title: "eBooks",
                        subtitle: [
                            "3 high-probability trading setups",
                            "knowledge checks",
                            "Assignments",
                            "Economic calendar",
                            "Cryptocurrency calendar",
                            "Daily market news",
                        ],
                    },
                    {
                        title: "31 lessons",
                    },
                ],
            },
            {
                price: "£189",
                discount: "£250",
                head: "Beginner",
                checks: [
                    {
                        title: "Trading essentials course",
                        subtitle: [
                            "Technical trading couse",
                            "Strategic trading course",
                            "MT4 & MT5 tutorials",
                        ],
                    },
                    {
                        title: "eBooks",
                        subtitle: [
                            "3 high-probability trading setups",
                            "knowledge checks",
                            "Assignments",
                            "Economic calendar",
                            "Cryptocurrency calendar",
                            "Daily market news",
                        ],
                    },
                    {
                        title: "31 lessons",
                    },
                ],
            },
            {
                price: "£189",
                discount: "£250",
                head: "Beginner",
                checks: [
                    {
                        title: "Trading essentials course",
                        subtitle: [
                            "Technical trading couse",
                            "Strategic trading course",
                            "MT4 & MT5 tutorials",
                        ],
                    },
                    {
                        title: "eBooks",
                        subtitle: [
                            "3 high-probability trading setups",
                            "knowledge checks",
                            "Assignments",
                            "Economic calendar",
                            "Cryptocurrency calendar",
                            "Daily market news",
                        ],
                    },
                    {
                        title: "31 lessons",
                    },
                ],
            },
            {
                price: "£189",
                discount: "£250",
                head: "Beginner",
                checks: [
                    {
                        title: "Trading essentials course",
                        subtitle: [
                            "Technical trading couse",
                            "Strategic trading course",
                            "MT4 & MT5 tutorials",
                        ],
                    },
                    {
                        title: "eBooks",
                        subtitle: [
                            "3 high-probability trading setups",
                            "knowledge checks",
                            "Assignments",
                            "Economic calendar",
                            "Cryptocurrency calendar",
                            "Daily market news",
                        ],
                    },
                    {
                        title: "31 lessons",
                    },
                ],
            },
        ],
    },
];

export const feedback = [
    {
        pic: '/assets/images/other/2.png',
        name: "Name Surname",
        position: "position",
        comment:
            "I like how they teach you how you can mitigate the risks associated with trading",
        date: "27 January",
        time: "23:15",
    },
    {
        pic: '/assets/images/other/2.png',
        name: "Name Surname",
        position: "position",
        comment:
            "I easily managed to finish the complete course and have been able to apply the principles immediately to improve the profitability of my trades.",
        date: "27 January",
        time: "23:15",
    },
    {
        pic: '/assets/images/other/2.png',
        name: "Name Surname",
        position: "position",
        comment: "The strategic course is my favorite!",
        date: "27 January",
        time: "23:15",
    },
    {
        pic: '/assets/images/other/2.png',
        name: "Name Surname",
        position: "position",
        comment:
            "I easily managed to finish the complete course and have been able to apply the principles immediately to improve the profitability of my trades.",
        date: "27 January",
        time: "23:15",
    },
    {
        pic: '/assets/images/other/2.png',
        name: "Name Surname",
        position: "position",
        comment:
            "I like how they teach you how you can mitigate the risks associated with trading",
        date: "27 January",
        time: "23:15",
    },
    {
        pic: '/assets/images/other/2.png',
        name: "Name Surname",
        position: "position",
        comment: "The strategic course is my favorite!",
        date: "27 January",
        time: "23:15",
    },
];

export const courses = [
    {
        title: "Trading essentials course",
        paid: true,
        price: "189 ",
    },
    {
        title: "Trading rg adrg course",
        paid: true,
        price: "90 ",
    },
    {
        title: "Tdsfg dfg rading essentials course",
        paid: false,
        price: "1333 ",
    },
    {
        title: "Trading  course",
        paid: true,
        price: "545 ",
    },
    {
        title: "Trading essentials course",
        paid: false,
        price: "76 ",
    },
];
